...
namespace Granity.winTools
{
    ...
    {
        private DataGridView TheDataGridView; // 要打印的 DataGridView 
        private PrintDocument ThePrintDocument; // 打印控件
        private bool IsCenterOnPage; // 是否在纸的中间打印
        private bool IsWithTitle; // 是否包含标题
        private string TheTitleText; // 标题的内容
        private Font TheTitleFont; //字体
        private Color TheTitleColor; // 颜色
        ...
        {
            TheDataGridView = aDataGridView;//数据源
            ThePrintDocument = aPrintDocument;//打印控件
            IsCenterOnPage = CenterOnPage;
            IsWithTitle = WithTitle;
            TheTitleText = aTitleText;//标题
            TheTitleFont = aTitleFont;//标题字体
            TheTitleColor = aTitleColor;//标题文本颜色
        ...
        private void Calculate(Graphics g)
        {
            if (PageNumber == 0)
            {
                ...
                for (int i = 0; i < TheDataGridView.Columns.Count; i++)
                {
                    ...
                    if (tmpFont == null) // 如果标题没有样式，我们就用它默认的样式
